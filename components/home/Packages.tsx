...
import { packages } from "@/lib/data/homepage-content";
    ...
    <section className="bg-navy-800 py-15 md:py-[80px] xl:py-30">
      <div className="mx-auto max-w-[1440px] px-5 md:px-12 xl:px-20">
        <Reveal className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
          ...
          >
            {packages.allLink.label} →
          </Link>
        </Reveal>

        <p className="mt-4 text-caption text-text-faint italic">{packages.note}</p>
              ...
              <Reveal key={item.code} delay={i * 80}>
                <Card href={item.href} className="h-full flex flex-col">
                  <div className="relative">
                    <PlaceholderImage asset={media} className="h-[180px] md:h-[200px]" />
                    ...
                    <span className="font-mono-brand absolute bottom-3 right-3 text-[10px] uppercase tracking-[0.08em] text-white bg-brand/0 group-hover:bg-brand/80 rounded px-2 py-1 transition-colors">
                      REALITY / XR VIEW
                    </span>
                  </div>
                  <div className="p-5 flex flex-col flex-1">
                    <h3 className="text-h3 font-semibold text-white">{item.name}</h3>
                    <p className="mt-2 text-body-sm leading-[1.6] text-text-muted flex-1">
                      {item.body}
                    </p>
                    <p className="mt-3 text-caption text-text-faint">適用情境：{item.useCase}</p>
